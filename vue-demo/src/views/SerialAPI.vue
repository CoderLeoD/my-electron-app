<template>
  <div>
    <el-row type="flex" class="row-bg" justify="center" v-show="portsList.length == 0">
      <el-col :span="7"
    ...
    <el-form v-show="portsList.length > 0" ref="form" :model="form" label-width="100px">
      <el-row>
        <el-col :span="6"
          ><div>
  ...
  methods: {
    ...
    callBack(value) {
      if (this.form.isShowHistory) this.form.desc = this.readLi().join('')
        ...
        if (value.length > 0) this.form.desc = this.myserialport.hex2atostr(value)
      }
    },
    clearHistory() {
      this.form.desc = ''
